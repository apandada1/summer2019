...
The following graphs show that the energy in rotating coordinates are different, and the ratio of average energies is very close to the ratio of $T^*_1$ and $T^* _2$ obtained from (\ref{eq:eff_temp}). Also note that this ratio is very different from $e^{4r}$. However, this is in accord with the relation between $r$ and $\tilde{r}$ obtained in the previous section.
...
In \ref{fluctuation_dissipation_proof} we saw that the effective temperature of the system is proportional to the square of the magnitude of the stochastic force. The force $\xi(t) =  a_0 [e^r \xi_1(t) \cos(\omega t) + e^{-r} \xi_2(t)\sin(\omega t)]$ can be viewed as a combination of two forces with magnitudes $a_1 = a_0 e^r$ and $a_2 = a_0 e^{-r}$. This is as if, the system is simultaneously coupled to two heat baths of temperatures $T e^{2r}$ and $T e^{-2r}$. This gives rise to the different temperatures in the two rotating coordinates. Note that the temperatures in (\ref{eq:eff_temp}) are just linear combinations of the temperatures of these two heat baths.
