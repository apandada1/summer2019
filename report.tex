...
I am grateful to Prof. H. R. Krishnamurthy for guiding me in this project and growing my interest in different branches of Condensed Matter Physics. 

I am also grateful to D.S.T. and K.V.P.Y. for providing my scholarship.
\tableofcontents
...
The following graphs show that the energy in rotating coordinates are different, and the ratio of average energies is very close to the ratio of $T^*_1$ and $T^* _2$ obtained from (\ref{eq:eff_temp}). This also shows that $r$ and $\tilde{r}$ are not equal and the relation between them is given by the expression in the previous section.
...
There is a notion of generalized Carnot limit given by $\eta = 1 - \frac{T^*_{high}}{T^*_{low}}$ (where $T^*$ denotes the effective temperature). It is seen that the experimental efficiency is still less than this limit, although it is more than the standard Carnot limit $1 - \frac{T_{high}}{T_{low}}$. 

When we calculate the change in entropy, we need to take into account the effective temperatures  ${T^*_{high}}$ and ${T^*_{low}}$ of the heat baths, and that is why, the second law of thermodynamics should not be violated. When we use this special stochastic force, the system is really coupled to two heat baths of different temperatures, and we really would not compare its efficiency to the ``formula" of standard Carnot efficiency, with some intermediate temperature $T_{high}$ and $T_{low}$.
